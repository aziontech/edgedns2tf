...
class TerraformStateGenerator:
    ...
    def _terraform_mk_conf(self):
        with open(f"./{self.file_name}_tmp.tf", "w") as f:
            f.write(constants.TERRAFORM_INITAL_CONTENT)
            f.write(constants.TERRAFORM_IDNS_DATA)
    ...
    def _init_context(self):
        subprocess.run(["terraform", "init"], encoding="utf-8", stdout=subprocess.DEVNULL)
        subprocess.run(["terraform", "apply"], encoding="utf-8", stdout=subprocess.DEVNULL)
    ...
    def _parse_state_output(self):
        ...
        for zone_id in output_values:
            ...
            state = 'data.azion_intelligent_dns_records.records["{}"]'.format(zone_id)
    ...
    def _save_terraform_config(self):
        with open(self.file_name+".tf", "w") as f:
            f.write(constants.TERRAFORM_INITAL_CONTENT)
            f.write(self.zone_code)
            f.write(self.records_code)

        with open(self.file_name+".sh", "w") as f:
            f.write(self.import_idns)
